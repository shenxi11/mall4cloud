<!--
模块名: platform_ai_operation_page
功能概述: 平台端运营 AI 占位页面，提供场景选择和模拟生成入口。
对外接口: 动态路由 component=platform/ai-operation
依赖关系: api/platform/ai-operation, Element Plus
...
<template>
  <div class="page-ai-operation">
    <el-alert
      title="AI服务未配置"
      type="warning"
      description="当前页面返回占位结果，后续配置模型服务后可用于商品标题、短视频脚本和直播话术生成。"
      show-icon
      :closable="false"
    />
      ...
      <div class="panel">
        ...
        >
          ...
          <el-descriptions-item label="建议提示词">
            {{ result.suggestedPrompt || activeScenePrompt }}
          </el-descriptions-item>
        </el-descriptions>
...
<script setup>
import * as api from '@/api/platform/ai-operation'
...
const activeScenePrompt = computed(() => {
  const scene = Data.scenes.find(item => item.scene === Data.form.scene)
  return scene ? scene.suggestedPrompt : '-'
})
