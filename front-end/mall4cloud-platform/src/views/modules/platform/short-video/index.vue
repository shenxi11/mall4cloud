<!--
模块名: platform_short_video_page
功能概述: 平台端短视频管理页面，提供列表、筛选、编辑和删除能力。
对外接口: 动态路由 component=platform/short-video
依赖关系: api/platform/short-video, Element Plus, img-upload
输入输出: 输入运营人员表单数据，输出短视频管理请求。
异常与错误: 接口异常由全局 request 拦截器提示，表单缺失由本页校验。
维护说明: 点播服务商未绑定前，视频地址按人工录入处理。
-->
<template>
    ...
    >
      ...
      >
        ...
        <el-form-item label="互动数据">
          <el-input-number
            v-model="dataForm.viewCount"
            :min="0"
            :precision="0"
            controls-position="right"
          />
          <el-input-number
            v-model="dataForm.likeCount"
            :min="0"
            :precision="0"
            controls-position="right"
            class="metric-input"
...
<script setup>
import * as api from '@/api/platform/short-video'
